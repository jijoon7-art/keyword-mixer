...
const FOODS: Food[] = [
  // ← 원본 food_worldcup_v3.html에서 const FOODS = [ ... ]; 전체를 복사해서 여기에 붙여넣으세요
  // (시간이 없으면 아래 예시로 먼저 테스트)
    ...
    <div className="max-w-[480px] mx-auto min-h-screen bg-[#F7F6F2] pb-12 text-[#1C1C1A]">
      ...
      {screen === 'home' && (
        <div className="px-5 pt-8 text-center">
          ...
          <button onClick={() => setScreen('setup')} className="w-full py-4 bg-orange-600 text-white rounded-2xl text-lg font-bold mb-4">
            🎯 상황에 맞춰 추천받기
          </button>
          <button onClick={startTournament} className="w-full py-4 bg-white border-2 border-orange-600 text-orange-600 rounded-2xl text-lg font-bold">
            🏆 바로 월드컵 시작
          </button>
        </div>
      )}
      ...
      {screen === 'ai' && aiRecommended && (
        <div className="p-6 text-center">
          ...
          <button onClick={() => setScreen('home')} className="w-full py-4 bg-gray-800 text-white rounded-2xl">
            처음으로
          </button>
        </div>
      )}
        ...
        <div className="p-5 text-center">
          ...
          <div className="space-y-4">
            <div onClick={() => handleSelectFood(matches[currentMatchIndex])} 
                 className="bg-white p-6 rounded-2xl border cursor-pointer hover:border-orange-500">
      ...
      {screen === 'result' && winner && (
        <div className="p-6 text-center">
          ...
          <button onClick={() => setScreen('home')} className="w-full py-4 bg-orange-600 text-white rounded-2xl">
            다시 하기
          </button>
        </div>
      )}
